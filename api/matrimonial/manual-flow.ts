...
function incomeNumber(v: any) {
  ...
  const text = clean(v).replace(/,/g, ""), nums = text.match(/\d+(?:\.\d+)?/g)?.map(Number) || [];
  if (!nums.length) return 0;
  let n = Math.max(...nums); if (/\bk\b/i.test(text)) n *= 1000; if (/\bm\b/i.test(text)) n *= 1000000; return n;
}
...
async function previewBatch(requesterProfileId: string) {
  ...
  return targets.map((target) => ({
    id: target.id, profileCode: code(target.id), name: target.name, gender: target.gender, age: target.age, city: target.city, country: target.country || "Pakistan",
    ...
    ...compatibility(requester, target),
...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === "GET") {
      const action = String(req.query?.action || "manual_consent_queue");
      if (action !== "manual_consent_queue") return send(res, 400, { error: "Unsupported action" });
      const rows = await prisma.$queryRaw<any[]>`
        ...
        LIMIT 200`;
      return send(res, 200, { requests: rows.map((r) => ({ ...r, requesterCode: code(r.requesterProfileId), targetCode: code(r.targetProfileId) })) });
    }
    if (req.method !== "POST") return send(res, 405, { error: "Method not allowed" });
    const body = await bodyOf(req), action = String(body.action || "");

    if (action === "preview_batch") {
      const requesterProfileId = clean(body.requesterProfileId);
      if (!requesterProfileId) return send(res, 400, { error: "Requester profile is required" });
      return send(res, 200, { matches: await previewBatch(requesterProfileId) });
    }
    ...
    if (action === "create_interest") {
      ...
      if (!requesterProfileId || !targetProfileId) return send(res, 400, { error: "Requester and target profiles are required" });
      const requester = await getProfile(requesterProfileId), target = await getProfile(targetProfileId);
      if (!requester || !target) return send(res, 404, { error: "Candidate profile not found" });
      if (requester.id === target.id || requester.gender === target.gender) return send(res, 400, { error: "These profiles cannot be paired" });
      for (const p of [requester, target]) if (p.status !== "approved" || p.showOnPortal !== true || p.isActive === false) return send(res, 409, { error: `${code(p.id)} must be approved and enabled for matching first.` });
      ...
      if (existing.length) return send(res, 409, { error: "An active interest already exists between these profiles." });
      ...
      return send(res, 201, { request: created[0], compatibility: c });
    }
    ...
    if (action === "record_consent") {
      ...
      if (!requestId || !["accept", "decline"].includes(decision)) return send(res, 400, { error: "Request and consent decision are required" });
      if (!["office_visit", "phone", "signed_form", "family_meeting", "other"].includes(consentMethod)) return send(res, 400, { error: "Select how consent was recorded" });
      if (note.length < 5) return send(res, 400, { error: "Add a short consent note for the audit trail" });
      ...
      const row = rows[0]; if (!row) return send(res, 404, { error: "Interest request not found" });
      if (row.status !== "awaiting_target") return send(res, 409, { error: "This request is not waiting for target consent" });
      if (row.targetAuthUserId) return send(res, 409, { error: "This target has a verified portal account. Consent should normally be given from the candidate account, not recorded manually." });
      ...
      return send(res, 200, { success: true, status: accepted ? "accepted" : "declined", photoReleased: photoRelease, contactReleased: contactRelease });
    }

    return send(res, 400, { error: "Unsupported action" });
  } catch (error: any) {
    console.error("manual matrimonial flow error", error);
    return send(res, Number(error?.status || 500), { error: error?.message || "Manual matrimonial workflow failed" });
  }
}
