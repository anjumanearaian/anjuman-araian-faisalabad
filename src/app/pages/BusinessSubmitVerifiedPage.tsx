...
export function BusinessSubmitVerifiedPage() {
  ...
  const validate = () => {
    ...
    if (!form.paymentProofUrl) next.paymentProofUrl = "Payment slip / receipt is required.";
    setErrors(next);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try { await createBusiness(form as any); setDone(true); window.scrollTo({ top: 0, behavior: "smooth" }); }
    catch (error: any) { setErrors({ form: error?.message || "Business submission failed." }); window.scrollTo({ top: 250, behavior: "smooth" }); }
    finally { setLoading(false); }
  };

  if (done) return <div><PageHeader title="Submission Received" breadcrumb={["Home","Business Directory","Register"]}/><section style={{maxWidth:600,margin:"65px auto",padding:"0 22px",textAlign:"center"}}><CheckCircle size={48} color={GREEN}/><h2 style={{color:GREEN,fontFamily:"'Playfair Display', serif"}}>Business Profile Submitted</h2><p style={{color:"#555",lineHeight:1.8}}>Your business profile and payment slip have been received. The listing is now <strong>Pending Review</strong>. The admin/finance team will verify the uploaded slip and approve the business before it appears in the public directory.</p><Link to="/business" style={button}>Back to Directory</Link></section></div>;

  return <div><PageHeader title="Register Business" subtitle="Submit your business profile and payment slip for admin review" breadcrumb={["Home","Business Directory","Register"]}/><section style={{maxWidth:850,margin:"0 auto",padding:"40px 22px 70px"}}>
    ...
    <div style={{background:"#fff9e9",border:"1px solid #ead39a",borderRadius:12,padding:18,marginBottom:20}}><h3 style={{margin:"0 0 7px",color:GREEN,fontFamily:"'Playfair Display', serif",display:"flex",gap:7,alignItems:"center"}}><DollarSign size={18} color={GOLD}/>Payment Slip Verification</h3><p style={{margin:"0 0 12px",fontSize:12,color:"#665c45",lineHeight:1.7}}>Pay through an official payment method and upload the payment slip or receipt. A transaction/reference number is not required. The admin/finance team will verify the uploaded proof before approving the listing.</p><div className="pay-methods" style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:9}}>{settings.paymentMethods?.length?settings.paymentMethods.map((pm:any)=><div key={pm.id} style={{background:"white",padding:10,borderRadius:8,fontSize:11}}><b style={{color:GREEN}}>{pm.bankName}</b><div>Title: {pm.accountTitle}</div><strong>{pm.accountNo}</strong></div>):<div style={{fontSize:12,color:"#777"}}>No payment method configured. Contact the office before paying.</div>}</div></div>
    <form onSubmit={submit} style={{background:"white",border:"1px solid #e8e3da",borderRadius:14,padding:26,boxShadow:"0 5px 24px rgba(0,0,0,.05)"}}>
      ...
      {Object.keys(errors).filter(k=>k!=="form").length>0&&<div style={{background:"#fff7ed",color:"#9a3412",padding:11,borderRadius:8,marginBottom:14,fontWeight:700,fontSize:12}}>Please complete the highlighted required fields before submission.</div>}
      <Heading icon={<Briefcase size={17}/>} text="Business Information"/><div className="form-grid" style={grid}>
        <Field title="Business Name *" error={errors.businessName}><input style={errorInput(errors.businessName)} value={form.businessName} onChange={(e)=>set("businessName",e.target.value)}/></Field>
        <Field title="Owner / Member Name *" error={errors.ownerName}><input style={errorInput(errors.ownerName)} value={form.ownerName} onChange={(e)=>set("ownerName",e.target.value)}/></Field>
        ...
        <Field title="City *" error={errors.city}><input style={errorInput(errors.city)} value={form.city} onChange={(e)=>set("city",e.target.value)}/></Field>
        <Field title="Phone *" error={errors.phone}><input style={errorInput(errors.phone)} value={form.phone} onChange={(e)=>set("phone",e.target.value)}/></Field>
        <Field title="WhatsApp"><input style={input} value={form.whatsapp} onChange={(e)=>set("whatsapp",e.target.value)}/></Field>
        <Field title="Email *" error={errors.email}><input type="email" style={errorInput(errors.email)} value={form.email} onChange={(e)=>set("email",e.target.value)}/></Field>
        <Field title="Website"><input style={input} value={form.website} onChange={(e)=>set("website",e.target.value)}/></Field>
        <div style={{gridColumn:"span 2"}}><Field title="Address *" error={errors.address}><input style={errorInput(errors.address)} value={form.address} onChange={(e)=>set("address",e.target.value)}/></Field></div>
        <div style={{gridColumn:"span 2"}}><Field title="Business Description *" error={errors.description}><textarea rows={4} style={{...errorInput(errors.description),resize:"vertical"}} value={form.description} onChange={(e)=>set("description",e.target.value)}/></Field></div>
      ...
      <Heading text="Payment Slip"/><div className="payment-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <Field title="Sender / Account-Holder Name (optional)"><input style={input} value={form.paymentSenderName} onChange={(e)=>set("paymentSenderName",e.target.value)} placeholder="If known"/></Field>
      ...
      <div className="upload-grid" style={grid}><UploadBox title="Business Logo (optional)" value={form.logoUrl} loading={uploading} accept="image/*" onChange={upload("logoUrl")}/><UploadBox title="Payment Slip / Receipt *" value={form.paymentProofUrl} loading={uploading} accept="image/*,.pdf,application/pdf" onChange={upload("paymentProofUrl")} error={errors.paymentProofUrl}/></div>
      ...
      <p style={{background:"#f0f7f3",padding:11,borderRadius:8,color:"#526159",fontSize:12,lineHeight:1.7}}><strong>Review process:</strong> your submission remains pending until the admin/finance team checks the uploaded slip and approves the business profile.</p>
...
function Field({title,error,children}:{title:string;error?:string;children:React.ReactNode}){return <div data-form-error={error?"true":undefined}><label style={label}>{title}</label>{children}{error&&<div style={{color:"#b91c1c",fontSize:11,marginTop:4}}>{error}</div>}</div>}
...
function UploadBox({title,value,loading,accept,onChange,error}:{title:string;value:string;loading:boolean;accept:string;onChange:(e:React.ChangeEvent<HTMLInputElement>)=>void;error?:string}){const pdf=value.toLowerCase().includes(".pdf");return <div data-form-error={error?"true":undefined}><label style={label}>{title}</label><label style={{minHeight:110,border:`2px dashed ${error?"#b91c1c":value?GOLD:"#ccd8cf"}`,borderRadius:9,display:"grid",placeItems:"center",padding:10,cursor:"pointer",background:value?"#fff9ef":"#fafbf9",textAlign:"center"}}>{loading?<span>Uploading...</span>:value?(pdf?<div style={{color:GREEN,fontWeight:800}}><CheckCircle size={20}/><div>PDF uploaded</div></div>:<img src={value} alt="Uploaded" style={{maxWidth:"100%",maxHeight:90,objectFit:"contain"}}/>):<div><Upload size={21} color="#999"/><div style={{fontSize:11,color:"#888"}}>Click to upload</div></div>}<input type="file" accept={accept} style={{display:"none"}} onChange={onChange}/></label>{error&&<div style={{color:"#b91c1c",fontSize:11,marginTop:4}}>{error}</div>}</div>}
function errorInput(error?:string):React.CSSProperties{return error?{...input,border:"1px solid #b91c1c",boxShadow:"0 0 0 2px rgba(185,28,28,.08)"}:input}
