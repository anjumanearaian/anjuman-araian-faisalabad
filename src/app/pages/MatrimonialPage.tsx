...
function fromProfile(p: MatrimonialProfile): FormState {
  ...
  const legacyTimeline = parseLegacyTimeline(profile.marriageTimeline);
  return {
    ...
    contact: p.contact || "", familySetup: profile.familySetup || "", sect: profile.sect || "Prefer not to specify", languages: Array.isArray(profile.languages) ? profile.languages.join(", ") : "", hobbies: profile.hobbies || "", familyNotes: profile.familyNotes || p.familyBackground || "", relocation: profile.relocation || "",
    timelineValue:profile.marriageTimelineValue?String(profile.marriageTimelineValue):legacyTimeline.value, timelineUnit:(profile.marriageTimelineUnit||legacyTimeline.unit) as TimelineUnit, timelineFlexibility:(profile.marriageTimelineFlexibility||legacyTimeline.flexibility) as TimelineFlexibility, behaviorAnswers:{...blankBehaviorAnswers(),...(profile.behavior||{})},
...
export function MatrimonialPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setForm(draftData ? { ...next, ...draftData, behaviorAnswers:{...next.behaviorAnswers,...(draftData.behaviorAnswers||{})}, profileId: requestedId || draftData.profileId || "" } : next);
  ...
  const setBehavior = (key:BehaviorKey,value:number|undefined) => setForm((old) => ({...old,behaviorAnswers:{...old.behaviorAnswers,[key]:value}}));
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const profileData = {
      ...
      relocation: form.relocation, employmentType: form.employmentType, employerType: form.employerType, incomeBand: form.incomeBand, nationality: form.nationality, residenceStatus: form.residenceStatus,
    ...
    try {
      const saved = await createMatrimonial({
        ...
        heightCm: form.heightCm ? Number(form.heightCm) : null, maritalStatus: form.maritalStatus, country: form.country, province: form.country === "Pakistan" ? form.province : form.province || undefined, city: form.city,
  ...
  return <div>
    <PageHeader title={form.profileId ? "Edit Candidate Profile" : "Create Candidate Profile"} subtitle="Structured profile, layered preferences and privacy-first matching" breadcrumb={["Home", "Matrimonial", form.profileId ? "Edit Profile" : "New Profile"]}/>
    <section style={{ maxWidth: 1000, margin: "0 auto", padding: "30px 20px 72px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap", marginBottom: 14 }}><Link to="/matrimonial" style={{ color: GREEN, textDecoration: "none", fontWeight: 800, fontSize: 12, display: "inline-flex", alignItems: "center", gap: 5 }}><ArrowLeft size={14}/> Dashboard</Link><span style={{ color: "#778078", fontSize: 11 }}>{saveState}</span></div>
      <ReadinessPanel readiness={readiness} title="Your Profile & Match Readiness"/>
      ...
      <form onSubmit={submit} style={card}>
        ...
        <Section n="2" title="Location, Education & Career" subtitle="For Pakistan-based and overseas candidates across education and professional backgrounds."/>
        <div className="grid2" style={grid2}>
        ...
        <Section n="3" title="Family, Lifestyle & Marriage Intentions" subtitle="Short structured details improve compatibility without making the form unnecessarily complicated."/>
        <div className="grid2" style={grid2}>
          ...
          <Field label="Hobbies / Interests"><input placeholder="Reading, sports, travel..." style={input} value={form.hobbies} onChange={(e) => set("hobbies", e.target.value)}/></Field>
        ...
        <div style={{marginTop:11}}><BehaviorQuestionnaire answers={form.behaviorAnswers} onChange={setBehavior}/></div>
        ...
        <div className="grid2" style={grid2}>
          <RangePreference title="Preferred Age" min={form.ageMin} max={form.ageMax} importance={form.ageImportance} onMin={(v:string) => set("ageMin", v)} onMax={(v:string) => set("ageMax", v)} onImportance={(v:string) => set("ageImportance", v)} error={errors.ageMax}/>
          <RangePreference title="Preferred Height (cm)" min={form.heightMin} max={form.heightMax} importance={form.heightImportance} onMin={(v:string) => set("heightMin", v)} onMax={(v:string) => set("heightMax", v)} onImportance={(v:string) => set("heightImportance", v)} error={errors.heightMax}/>
        </div>
