...
export function AdminMemberCenterPage() {
  ...
  const startEdit = (m: Member) => {
    ...
    setForm({
      ...
      localArea: m.localArea || "", city: m.city || "", district: m.district || "", province: m.province || "Punjab",
  ...
  const approve = async (m: Member) => {
    if (!confirm(`Approve membership for ${m.fullName}? Payment must already be verified/recorded.`)) return;
  ...
  const assignLeadership = async () => {
    ...
    try {
      await apiClient("/leadership/profiles", { method: "POST", body: JSON.stringify({ memberId: m.id, name: m.fullName, role: assignment.role, city: m.city || "Faisalabad", tier: Number(assignment.tier) || 2, category: assignment.category, image: m.photoUrl || null, period: assignment.period || null }) });
      setMessage(`${m.fullName} assigned to ${assignment.role}.`);
        ...
        {view === "members" && <>
          ...
          <section style={panelStyle}>
            ...
            <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1120 }}><thead><tr style={{ background: "#f8f5ef" }}>{["Form / Registration", "Member", "Location", "Status", "Payment", "Publishing", "Actions"].map((h) => <th key={h} style={thStyle}>{h}</th>)}</tr></thead><tbody>{filtered.map((m) => <tr key={m.id} style={{ borderBottom: "1px solid #eee" }}>
              ...
              <td style={tdStyle}>{m.city || "—"}<br /><span style={{ fontSize: 11, color: "#777" }}>{[m.localArea,m.district].filter(Boolean).join(" · ")}</span></td>
          ...
          <section style={{ ...panelStyle, marginTop: 20 }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}><Crown size={18} color={GOLD} /><h2 style={{ color: GREEN, fontFamily: "'Playfair Display', serif", margin: 0, fontSize: 19 }}>Cabinet / Committee Assignment</h2></div><p style={{ color: "#777", fontSize: 12, margin: "0 0 16px" }}>Assign only approved members. The leadership record remains linked to the membership database.</p><div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr .5fr 1fr auto", gap: 10 }} className="assign-grid"><select value={assignment.memberId} onChange={(e) => setAssignment((a) => ({ ...a, memberId: e.target.value }))} style={fieldStyle}><option value="">Select approved member</option>{members.filter((m) => m.status === "approved").map((m) => <option key={m.id} value={m.id}>{m.fullName} · {m.memberNo}</option>)}</select><input value={assignment.role} onChange={(e) => setAssignment((a) => ({ ...a, role: e.target.value }))} placeholder="Role" style={fieldStyle} /><select value={assignment.category} onChange={(e) => setAssignment((a) => ({ ...a, category: e.target.value }))} style={fieldStyle}><option value="cabinet">Cabinet</option><option value="executive">Executive Committee</option><option value="advisory">Advisory Board</option><option value="founder">Founder</option><option value="expresident">Ex-President</option></select><input type="number" min="0" max="10" value={assignment.tier} onChange={(e) => setAssignment((a) => ({ ...a, tier: e.target.value }))} style={fieldStyle} /><input value={assignment.period} onChange={(e) => setAssignment((a) => ({ ...a, period: e.target.value }))} placeholder="2026–2028" style={fieldStyle} /><button onClick={() => void assignLeadership()} style={primaryButton}><UserCog size={14} /> Assign</button></div></section>
        </>}
...
function MemberFormModal({ form, setForm, selected, allMembers, saving, onClose, onSave }: { form: Record<string, any>; setForm: (v: any) => void; selected: Member | null; allMembers: Member[]; saving: boolean; onClose: () => void; onSave: () => void }) {
  ...
  const onCity = (city: string) => { const hint = suggestLocation(city); setForm((p: any) => ({ ...p, city, district: hint.district || p.district, province: hint.province || p.province })); };
  return <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,.55)", display: "grid", placeItems: "center", padding: 16 }}><div style={{ background: "white", borderRadius: 14, width: "min(980px,96vw)", maxHeight: "92vh", overflow: "auto", boxShadow: "0 20px 70px rgba(0,0,0,.25)" }}>
    ...
    <div style={{ padding: 24 }}>
      ...
      <FormSection title="Contact & Location"><Grid><Field label="Email" type="email" value={form.email} onChange={(v) => set("email", v)} /><Field label="Phone *" value={form.phone} onChange={(v) => set("phone", v)} /><Field label="WhatsApp" value={form.whatsapp} onChange={(v) => set("whatsapp", v)} /><label style={labelWrap}>City / Town<input list="admin-cities" value={form.city || ""} onChange={(e) => onCity(e.target.value)} style={fieldStyle} /><datalist id="admin-cities">{pakistanMajorCities.map((x) => <option key={x} value={x} />)}</datalist></label><label style={labelWrap}>District<input list="admin-districts" value={form.district || ""} onChange={(e) => set("district", e.target.value)} style={fieldStyle} /><datalist id="admin-districts">{pakistanDistricts.map((x) => <option key={x} value={x} />)}</datalist></label><SelectField label="Province / Region" value={form.province} onChange={(v) => set("province", v)} options={provinces} /><Field label="Local Area / Tehsil / Village" value={form.localArea} onChange={(v) => set("localArea", v)} /><div style={{ gridColumn: "span 2" }}><Field label="Full Address" value={form.address} onChange={(v) => set("address", v)} /></div></Grid></FormSection>
