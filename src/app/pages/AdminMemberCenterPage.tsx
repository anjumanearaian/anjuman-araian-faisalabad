...
import { MemberDocumentsUpload } from "../components/admin/MemberDocumentsUpload";
...
const blankMember = () => ({
  ...
  membershipType: "ordinary", paymentStatus: "pending", status: "pending", showOnPortal: true, showOnWeb: false,
  photoUrl: "", cnicFrontUrl: "", cnicBackUrl: "", paymentProofUrl: "", additionalPhotos: [] as string[], adminNote: "",
...
export function AdminMemberCenterPage() {
  ...
  const startAdd = () => {
    setSelected(null); setForm(blankMember()); setShowForm(true); setMessage(""); setError("");
  };
  ...
  const startEdit = (m: Member) => {
    ...
    setForm({
      ...
      showOnPortal: Boolean(m.showOnPortal), showOnWeb: Boolean(m.showOnWeb),
      photoUrl: m.photoUrl || "", cnicFrontUrl: m.cnicFrontUrl || "", cnicBackUrl: m.cnicBackUrl || "", paymentProofUrl: m.paymentProofUrl || "",
      additionalPhotos: Array.isArray(m.additionalPhotos) ? m.additionalPhotos : [], adminNote: m.adminNote || "",
  ...
  const saveMember = async () => {
    ...
    const payload: Record<string, any> = {
      ...
      additionalPhotos: Array.isArray(form.additionalPhotos) ? form.additionalPhotos : [],
    ...
    try {
      if (selected) {
        ...
        setMessage("Member record and uploaded documents updated successfully. Changes are retained in the member audit trail.");
      } else {
        await createAdminMember({ ...payload, status: form.status });
        setMessage("Member added successfully with the same document structure used by the online registration form.");
      }
  ...
  return <div style={{ minHeight: "100vh", background: "#f8f5ef", fontFamily: "'Lato', sans-serif" }}>
    ...
    <main style={{ maxWidth: 1240, margin: "0 auto", padding: "26px 22px 60px" }}>
      ...
      {view === "members" && <>
        ...
        <section style={{ ...panelStyle, marginTop: 20 }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}><Crown size={18} color={GOLD} /><h2 style={{ color: GREEN, fontFamily: "'Playfair Display', serif", margin: 0, fontSize: 19 }}>Governance & Role Assignments</h2></div><p style={{ color: "#777", fontSize: 12, margin: "0 0 14px", lineHeight: 1.65 }}>Cabinet, committee, zone and special-purpose assignments are managed in one Governance & Operations Center. Every role links to this same approved Member ID, so no duplicate person record is created.</p><Link to="/admin/operations" style={{ ...primaryButton, textDecoration: "none", width: "fit-content" }}><UserCog size={14} /> Open Governance & Operations</Link></section>
      </>}
...
function MemberFormModal({ form, setForm, selected, allMembers, saving, onClose, onSave }: { form: Record<string, any>; setForm: (v: any) => void; selected: Member | null; allMembers: Member[]; saving: boolean; onClose: () => void; onSave: () => void }) {
  ...
  return <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,.55)", display: "grid", placeItems: "center", padding: 16 }}><div style={{ background: "white", borderRadius: 14, width: "min(980px,96vw)", maxHeight: "92vh", overflow: "auto", boxShadow: "0 20px 70px rgba(0,0,0,.25)" }}>
    ...
    <div style={{ padding: 24 }}>
      ...
      <FormSection title="Member Documents & Uploads"><MemberDocumentsUpload photoUrl={form.photoUrl} cnicFrontUrl={form.cnicFrontUrl} cnicBackUrl={form.cnicBackUrl} paymentProofUrl={form.paymentProofUrl} additionalFiles={Array.isArray(form.additionalPhotos) ? form.additionalPhotos : []} onChange={(key, value) => set(key, value)} /></FormSection>
      <FormSection title="Membership & Publishing"><Grid><MembershipTypeField value={form.membershipType} onChange={(v) => set("membershipType", v)} /><div style={{ display: "grid", alignContent: "end", color: "#777", fontSize: 11, lineHeight: 1.6 }}>Profile photo and supporting documents are managed in the Member Documents section above.</div><label style={checkBoxCard}><input type="checkbox" checked={Boolean(form.showOnPortal)} onChange={(e) => set("showOnPortal", e.target.checked)} /> Show to approved members in portal</label><label style={checkBoxCard}><input type="checkbox" checked={Boolean(form.showOnWeb)} onChange={(e) => set("showOnWeb", e.target.checked)} /> Show in public web directory</label><div style={{ gridColumn: "span 2" }}><Field label="Admin Note (Internal Only)" value={form.adminNote} onChange={(v) => set("adminNote", v)} /></div></Grid></FormSection>
