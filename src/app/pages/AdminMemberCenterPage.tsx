...
import { MemberRolesPanel } from "../components/admin/MemberRolesPanel";
import logo from "../../imports/logo.png";
import { ArrowLeft, CalendarDays, CheckCircle, Heart, MessageCircle, Plus, Printer, RefreshCw, Search, ShieldCheck, Trash2, UserCog, Users, Crown } from "lucide-react";
...
const leadershipRoles = Array.from(new Set([
  ...
  "Executive Member", "Chairman", "Convener", "Committee Secretary", "Committee Member", "Member", "Advisor", "Former President",
  ...designationSuggestions,
]));
...
export function AdminMemberCenterPage() {
  ...
  const alphabeticalApprovedMembers = useMemo(() => members
    .filter((m) => m.status === "approved")
    .slice()
    .sort((a, b) => a.fullName.localeCompare(b.fullName, "en", { sensitivity: "base" })), [members]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f8f5ef", fontFamily: "'Lato', sans-serif" }}>
      <header style={{ background: GREEN, color: "white", borderBottom: `4px solid ${GOLD}` }}><div style={{ maxWidth: 1240, margin: "0 auto", padding: "18px 22px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16, flexWrap: "wrap" }}><div style={{ display: "flex", alignItems: "center", gap: 14 }}><img src={logo} alt="Anjuman e Araian" style={{ width: 52, height: 52, objectFit: "contain", background: "white", borderRadius: 50, padding: 3 }} /><div><h1 style={{ margin: 0, fontFamily: "'Playfair Display', serif", fontSize: 23 }}>Member & Approval Center</h1><p style={{ margin: "4px 0 0", color: "rgba(255,255,255,.72)", fontSize: 12 }}>Anjuman-e-Araian Faisalabad</p></div></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Link to="/admin/governance" style={{ color: "white", textDecoration: "none", border: "1px solid rgba(255,255,255,.25)", padding: "8px 12px", borderRadius: 7, fontWeight: 700, fontSize: 12, display: "inline-flex", alignItems: "center", gap: 5 }}><CalendarDays size={14} /> Governance & Meetings</Link><Link to="/admin" style={{ color: "white", textDecoration: "none", border: "1px solid rgba(255,255,255,.25)", padding: "8px 12px", borderRadius: 7, fontWeight: 700, fontSize: 12, display: "inline-flex", alignItems: "center", gap: 5 }}><ArrowLeft size={14} /> Main Admin Panel</Link></div></div></header>
      <main style={{ maxWidth: 1240, margin: "0 auto", padding: "26px 22px 60px" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}><button onClick={() => setView("members")} style={tabButton(view === "members")}><Users size={15} /> Members & Approvals</button><button onClick={() => setView("matches")} style={tabButton(view === "matches")}><Heart size={15} /> Matrimonial Match Requests</button><Link to="/admin/governance" style={{ ...tabButton(false), textDecoration: "none" }}><CalendarDays size={15} /> Meetings & Attendance</Link></div>
        ...
        {view === "members" && <>
          ...
          <section style={{ ...panelStyle, marginTop: 20 }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}><Crown size={18} color={GOLD} /><h2 style={{ color: GREEN, fontFamily: "'Playfair Display', serif", margin: 0, fontSize: 19 }}>Cabinet / Committee Assignment</h2></div><p style={{ color: "#777", fontSize: 12, margin: "0 0 16px" }}>One approved member may hold multiple assignments. The same controls are also available inside each member’s Edit form.</p><div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1.2fr .5fr 1fr auto", gap: 10 }} className="assign-grid"><select value={assignment.memberId} onChange={(e) => setAssignment((a) => ({ ...a, memberId: e.target.value }))} style={fieldStyle}><option value="">Select approved member</option>{alphabeticalApprovedMembers.map((m) => <option key={m.id} value={m.id}>{m.fullName}</option>)}</select><label style={{ position: "relative" }}><input list="leadership-role-options" value={assignment.role} onChange={(e) => setAssignment((a) => ({ ...a, role: e.target.value }))} placeholder="Role / Designation" style={fieldStyle} /><datalist id="leadership-role-options">{leadershipRoles.map((x) => <option key={x} value={x} />)}</datalist></label><div style={{ display: "grid", gap: 6 }}><select value={assignment.category} onChange={(e) => setAssignment((a) => ({ ...a, category: e.target.value }))} style={fieldStyle}><option value="cabinet">Executive Council / Cabinet</option><option value="executive">Executive Body / Executive Committee</option><option value="management-committee">Management Committee</option><option value="standing-committee">Standing Committee</option><option value="finance-committee">Finance Committee</option><option value="advisory">Advisory Board</option><option value="women-wing">Women Wing</option><option value="custom">New / Other Committee</option></select>{assignment.category === "custom" && <input value={assignment.customCategory} onChange={(e) => setAssignment((a) => ({ ...a, customCategory: e.target.value }))} placeholder="Committee name" style={fieldStyle} />}</div><input type="number" min="0" max="10" value={assignment.tier} onChange={(e) => setAssignment((a) => ({ ...a, tier: e.target.value }))} title="Display order / hierarchy tier" style={fieldStyle} /><input value={assignment.period} onChange={(e) => setAssignment((a) => ({ ...a, period: e.target.value }))} placeholder="2026–2028" style={fieldStyle} /><button onClick={() => void assignLeadership()} style={primaryButton}><UserCog size={14} /> Assign</button></div></section>
        </>}
...
function MemberFormModal({ form, setForm, selected, allMembers, saving, onClose, onSave }: { form: Record<string, any>; setForm: (v: any) => void; selected: Member | null; allMembers: Member[]; saving: boolean; onClose: () => void; onSave: () => void }) {
  ...
  const referrerOptions = allMembers
    .filter((m) => m.status === "approved" && m.id !== selected?.id)
    .slice()
