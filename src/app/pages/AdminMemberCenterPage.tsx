...
async function printMemberForm(m: Member) {
  ...
  const officePage = `<section class="sheet"><div class="copytag">OFFICE RECORD COPY</div><div class="head"><img class="logo" src="${logoUrl}"><div><div class="title">ANJUMAN-E-ARAIAN FAISALABAD</div><div class="subtitle">Official Membership Registration & Undertaking</div><div class="goldline"></div><div class="statusline"><span class="badge">${escapeHtml(String(m.status || "").toUpperCase())}</span><span>${escapeHtml(membershipLabel)}</span></div></div>${photo}</div><div class="idstrip">${row("Form No.", m.formNo)}${row("Registration / Member No.", m.memberNo)}${row("Membership", membershipLabel)}${row("Approved On", approvedDate)}</div><div class="section"><h3>Member Information</h3><div class="grid4">${row("Full Name", m.fullName)}${row("Father's Name", m.fatherName)}${row("CNIC", m.cnic)}${row("Date of Birth", m.dob)}${row("Mobile", m.phone)}${row("WhatsApp", m.whatsapp)}${row("City / Area", [m.city, m.localArea].filter(Boolean).join(" · "))}${row("Profession / Designation", [occ.base, m.designation].filter(Boolean).join(" · "))}</div><div class="fullrow">${row("Address", m.address)}</div></div><div class="section"><h3>Membership & Payment</h3><div class="grid4">${row("Membership Type", membershipLabel)}${row("Payment Status", m.paymentStatus)}${row("Receipt / Transaction No.", paymentNo)}${row("Amount", paymentAmount)}${row("Payment Date", paymentDate)}${row("Received By", receivedBy)}${row("Education", [edu.base, edu.detail].filter(Boolean).join(" · "))}${row("Institute / Organization", m.institutionName)}</div></div><div class="undertaking"><div class="undertitle">اقرار و حلفیہ بیان / DECLARATION & UNDERTAKING</div><div class="urdu">${escapeHtml(undertaking)}</div></div><div class="signatures"><div><div class="sigline"></div><b>Member Signature</b><small>Thumb impression if required</small></div><div><div class="sigline"></div><b>Payment Received By</b><small>${escapeHtml(receivedBy)}</small></div><div><div class="sigline"></div><b>Form Prepared / Verified By</b><small>Office verification</small></div><div><div class="sigline"></div><b>General Secretary</b><small>Signature & stamp</small></div><div><div class="sigline"></div><b>President / Authorized Signatory</b><small>Signature & stamp</small></div></div><div class="foot"><span>Member ID: ${escapeHtml(m.id)}</span><span>Generated: ${escapeHtml(generated)}</span><span>Subject to the Constitution and Rules of Anjuman-e-Araian Faisalabad.</span></div></section>`;

  const memberPage = `<section class="sheet membercopy"><div class="copytag">MEMBER COPY</div><div class="head"><img class="logo" src="${logoUrl}"><div><div class="title">ANJUMAN-E-ARAIAN FAISALABAD</div><div class="subtitle">Membership Acknowledgement</div><div class="goldline"></div><div class="statusline"><span class="badge">${escapeHtml(String(m.status || "").toUpperCase())}</span><span>${escapeHtml(membershipLabel)}</span></div></div>${photo}</div><div class="ack">This is to acknowledge that <strong>${escapeHtml(m.fullName)}</strong>, Registration No. <strong>${escapeHtml(m.memberNo || "—")}</strong>, is recorded as a <strong>${escapeHtml(membershipLabel)}</strong> member of Anjuman-e-Araian Faisalabad, subject to the Constitution, rules and current membership status of the Association.</div><div class="section"><h3>Membership Record</h3><div class="grid4">${row("Full Name", m.fullName)}${row("Father's Name", m.fatherName)}${row("CNIC", m.cnic)}${row("Mobile", m.phone)}${row("Form No.", m.formNo)}${row("Registration No.", m.memberNo)}${row("Membership Type", membershipLabel)}${row("Status", m.status)}${row("Receipt / Transaction No.", paymentNo)}${row("Amount", paymentAmount)}${row("Payment Date", paymentDate)}${row("Payment Received By", receivedBy)}</div></div><div class="membernote"><b>Membership Note</b><p>Membership remains subject to the Constitution, rules, approved fees and disciplinary procedures of Anjuman-e-Araian Faisalabad. The Association is a non-political community welfare body; personal political preferences or affiliations do not form part of its official membership record or decision-making.</p></div><div class="signatures member-sigs"><div><div class="sigline"></div><b>General Secretary</b><small>Signature & stamp</small></div><div><div class="sigline"></div><b>President / Authorized Signatory</b><small>Signature & stamp</small></div><div><div class="sigline"></div><b>Member Acknowledgement</b><small>Signature</small></div></div><div class="foot"><span>Form: ${escapeHtml(m.formNo || "—")}</span><span>Registration: ${escapeHtml(m.memberNo || "—")}</span><span>Generated: ${escapeHtml(generated)}</span></div></section>`;

  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(m.formNo || m.memberNo)} - Membership Document</title><style>@page{size:A4;margin:8mm}*{box-sizing:border-box}body{font-family:Arial,"Segoe UI",sans-serif;color:#1f2937;margin:0;background:#eef2ee}.sheet{position:relative;background:white;border:1.8px solid #164c32;border-top:8px solid #164c32;padding:12mm 10mm 9mm;min-height:281mm;page-break-after:always;overflow:hidden}.sheet:last-child{page-break-after:auto}.sheet:after{content:"";position:absolute;left:0;right:0;bottom:0;height:5px;background:#c9a34f}.copytag{position:absolute;right:10mm;top:4mm;font-size:8px;letter-spacing:.13em;color:#6b7280;font-weight:800}.head{display:grid;grid-template-columns:68px 1fr 82px;gap:12px;align-items:center;padding-bottom:9px}.logo{width:62px;height:62px;object-fit:contain}.photo{width:76px;height:92px;border:1px solid #cbd5ce;border-radius:5px;object-fit:cover;background:#fafafa}.placeholder{display:grid;place-items:center;color:#a3a3a3;font-size:9px}.title{color:#164c32;font-family:Georgia,"Times New Roman",serif;font-size:18px;font-weight:800;letter-spacing:.02em}.subtitle{font-size:10px;color:#606b65;margin-top:3px;font-weight:700;text-transform:uppercase;letter-spacing:.07em}.goldline{width:92px;height:3px;background:#c9a34f;margin:7px 0}.statusline{display:flex;gap:8px;align-items:center;font-size:9px;color:#315744;font-weight:700}.badge{padding:3px 8px;border-radius:20px;background:#e8f5ec;color:#17663b;font-size:8px}.idstrip{display:grid;grid-template-columns:1fr 1fr 1.25fr 1fr;background:#f8f5ec;border:1px solid #eadcae;padding:6px 9px;gap:12px;margin:4px 0 8px}.section{margin-top:8px}.section h3{font-size:9px;color:#164c32;text-transform:uppercase;letter-spacing:.12em;border-bottom:1px solid #dfe6e1;padding-bottom:4px;margin:0 0 4px}.grid4{display:grid;grid-template-columns:repeat(4,1fr);gap:0 12px}.fullrow{margin-top:1px}.field{padding:4px 0;border-bottom:1px dotted #ccd5cf;min-height:27px}.field span{display:block;font-size:6.6px;color:#7b837e;text-transform:uppercase;letter-spacing:.04em}.field strong{display:block;font-size:8.7px;font-weight:700;color:#26332c;margin-top:1px;word-break:break-word}.undertaking{margin-top:9px;border:1px solid #d8e1db;border-left:4px solid #c9a34f;background:#fbfcfa;padding:7px 9px}.undertitle{font-size:8px;color:#164c32;font-weight:800;text-align:center;letter-spacing:.05em;margin-bottom:5px}.urdu{font-family:"Noto Naskh Arabic","Noto Nastaliq Urdu","Segoe UI",Tahoma,Arial,sans-serif;direction:rtl;text-align:right;font-size:10.2px;line-height:1.75;color:#24342b}.signatures{display:grid;grid-template-columns:repeat(5,1fr);gap:9px;margin-top:13px;text-align:center}.signatures>div{font-size:7px;color:#4b5563}.signatures b{display:block;color:#213c2d;font-size:7.5px}.signatures small{display:block;font-size:6px;color:#7b837e;margin-top:2px}.sigline{height:24px;border-bottom:1px solid #53655a;margin-bottom:4px}.foot{position:absolute;left:10mm;right:10mm;bottom:6mm;border-top:1px solid #e4e8e5;padding-top:4px;display:flex;justify-content:space-between;gap:8px;font-size:6.2px;color:#7b837e}.ack{margin:12px 0;background:#f7f3e9;border:1px solid #e3d7ad;border-left:4px solid #c9a34f;padding:12px 14px;font-size:11px;line-height:1.65;color:#33423a}.membernote{margin-top:14px;border:1px solid #d9e2dc;border-radius:6px;padding:10px 12px;background:#fbfcfa}.membernote b{font-size:9px;color:#164c32;text-transform:uppercase;letter-spacing:.08em}.membernote p{margin:5px 0 0;font-size:9px;line-height:1.6;color:#4b5563}.member-sigs{grid-template-columns:repeat(3,1fr);margin-top:28px}.membercopy .head{margin-bottom:14px}@media print{body{background:white}.sheet{margin:0}button{display:none}}</style></head><body>${officePage}${memberPage}<script>window.onload=()=>setTimeout(()=>window.print(),500)</script></body></html>`;
