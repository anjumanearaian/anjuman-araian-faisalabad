...
type Band = "strong" | "good" | "broader";
function bandFor(score = 0): Band { return score >= 75 ? "strong" : score >= 55 ? "good" : "broader"; }
function scoreColor(score = 0) { return score >= 75 ? { bg: "#dcfce7", color: "#166534" } : score >= 55 ? { bg: "#fef3c7", color: "#92400e" } : { bg: "#f3f4f6", color: "#4b5563" }; }
...
export function MatrimonialMatchesPage() {
  ...
  const loadMatches = async (profileId: string) => {
    ...
    try {
      const res = await fetchMatrimonialMatches(profileId);
    ...
    <section style={{ maxWidth: 1120, margin: "0 auto", padding: "30px 20px 70px" }}>
      ...
      <div style={{ ...card, padding: 18, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
          <div><strong style={{ color: GREEN }}>Candidate profile</strong><p style={{ ...muted, margin: "4px 0 0" }}>Scores are calculated in both directions: your preferences against the other candidate and their preferences against yours.</p></div>
      ...
      <div style={{ background: "#eef6ff", border: "1px solid #cfe4fb", color: "#315f7d", borderRadius: 10, padding: 13, display: "flex", gap: 9, marginBottom: 16 }}><ShieldCheck size={17}/><div style={{ fontSize: 11, lineHeight: 1.65 }}><strong>Privacy layer:</strong> these cards use anonymous profile codes. Candidate name, photograph, phone number and sensitive family details are not included. Unknown fields lower score confidence rather than automatically counting as a mismatch.</div></div>
      {error && <Notice bg="#fee2e2" color="#b91c1c">{error}</Notice>}
      {success && <Notice bg="#dcfce7" color="#166534">{success}</Notice>}

      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 18 }}>{(["all","strong","good","broader"] as const).map((b) => <button key={b} onClick={() => setFilter(b)} style={{ border: `1px solid ${filter === b ? GREEN : "#d8ded9"}`, background: filter === b ? GREEN : "white", color: filter === b ? "white" : GREEN, borderRadius: 20, padding: "7px 11px", fontSize: 11, fontWeight: 800, cursor: "pointer", textTransform: "capitalize" }}>{b === "all" ? `All (${matches.length})` : `${b} (${matches.filter((m) => bandFor(m.mutualScore || 0) === b).length})`}</button>)}</div>

      {loading ? <div style={{ padding: 50, textAlign: "center", color: "#777" }}><Loader2 className="spin"/> Calculating private matches...</div> : visible.length ? <div className="match-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,minmax(0,1fr))", gap: 15 }}>{visible.map((m) => <MatchCard key={m.id} match={m} busy={busyId === m.id} onRequest={() => void request(m)} onDetails={() => setDetail(m)}/>)}</div> : <div style={{ ...card, textAlign: "center" }}><Sparkles size={30} color={GOLD}/><h3 style={{ color: GREEN }}>No matches in this band yet</h3><p style={muted}>Adding more structured partner preferences and keeping the candidate profile complete can improve matching quality as new profiles are approved.</p></div>}
    </section>

    {detail && <div style={modalBackdrop}><div style={modal}><button onClick={() => setDetail(null)} style={closeBtn}>×</button><h2 style={{ ...title, marginTop: 0 }}>{detail.profileCode} · {detail.mutualScore}% mutual compatibility</h2><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}><ScoreBox label="Your preferences → Candidate" value={detail.requesterToTargetScore || 0}/><ScoreBox label="Candidate preferences → You" value={detail.targetToRequesterScore || 0}/></div><div style={{ background: "#f8f5ef", borderRadius: 9, padding: 12, color: "#555", fontSize: 11, lineHeight: 1.6 }}>Score confidence: <strong>{detail.scoreConfidence || 0}%</strong>. This shows how much structured information was available for the comparison.</div>{detail.breakdown && <Breakdown data={detail.breakdown}/>}<div style={{ display: "flex", justifyContent: "flex-end", marginTop: 16 }}><button disabled={busyId === detail.id} onClick={() => void request(detail)} style={primaryBtn}><Send size={13}/> Send Private Interest</button></div></div></div>}
...
function MatchCard({ match: m, busy, onRequest, onDetails }: { match: MatrimonialProfile; busy: boolean; onRequest: () => void; onDetails: () => void }) {
  ...
  return <article style={{ ...card, padding: 19 }}>
    ...
    <div style={{ marginTop: 12, display: "flex", gap: 7, flexWrap: "wrap" }}>{m.eligible !== false ? <Tag good text="No Must-Have Conflict"/> : <Tag text="Broader Match"/>}<Tag text={`Confidence ${m.scoreConfidence || 0}%`}/>{m.verificationStatus && m.verificationStatus !== "unverified" && <Tag good text="Verified" icon/>}</div>
    <div style={{ display: "flex", gap: 8, marginTop: 15 }}><button onClick={onDetails} style={secondaryBtn}><ChevronDown size={13}/> Why this score?</button><button disabled={busy} onClick={onRequest} style={primaryBtn}><Heart size={13}/>{busy ? "Sending..." : "Show Interest"}</button></div>
  </article>;
}
...
function ScoreBox({ label, value }: { label:string; value:number }) { const c = scoreColor(value); return <div style={{ border: "1px solid #e8e3da", borderRadius: 9, padding: 12 }}><div style={{ color: "#777", fontSize: 10 }}>{label}</div><strong style={{ color: c.color, fontSize: 24 }}>{value}%</strong></div>; }
function Breakdown({ data }: { data: any }) { const left = data.requesterToTarget || []; const right = data.targetToRequester || []; return <details style={{ marginTop: 12, border: "1px solid #e8e3da", borderRadius: 9, padding: "0 11px" }}><summary style={{ padding: "10px 0", cursor: "pointer", color: GREEN, fontSize: 11, fontWeight: 800 }}>Compatibility breakdown</summary><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, paddingBottom: 12 }}><BreakColumn title="Your preferences" items={left}/><BreakColumn title="Their preferences" items={right}/></div></details>; }
