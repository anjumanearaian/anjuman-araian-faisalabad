...
import { ArrowLeft, CircleDollarSign, Download, Edit2, History, Landmark, Lock, Plus, ReceiptText, RefreshCw, RotateCcw, Save, ShieldCheck, Trash2, WalletCards, X } from "lucide-react";
import { useAdmin } from "../context/AdminContext";
import { smartSearchSort } from "../lib/searchUtils";
import {
  createFinanceHead, createFinanceTransaction, fetchFinanceAudit, fetchFinanceHeads, fetchFinanceLedger, fetchFinanceMembers,
  fetchFinanceOfficers, fetchFinanceSummary, financeReceiptUrl, FinanceAuditRow, FinanceHead, FinanceLedgerRow, FinanceLedgerView,
  FinanceMember, FinanceOfficer, FinanceSummary, restoreFinanceTransaction, updateFinanceTransaction, voidFinanceTransaction,
...
export function AdminFinancePage() {
  ...
  async function saveTransaction() {
    ...
    try {
      ...
      const wasEdit = Boolean(editingId); resetForm(); await load(); flash(wasEdit ? "Ledger entry corrected by Super Admin. The audit history was preserved." : "Ledger entry posted and locked with a permanent serial/document number.");
  ...
  return <div style={{minHeight:"100vh",background:BG,fontFamily:"Lato,Arial,sans-serif",color:"#333"}}>
    <style>{`@media(max-width:850px){.finance-main{padding:16px!important}.finance-grid{grid-template-columns:1fr!important}.finance-cards{grid-template-columns:1fr!important}.finance-table{min-width:1100px}.finance-header{padding:18px 16px!important}.finance-actions{width:100%}.member-results{position:static!important;max-height:260px!important}}`}</style>
    <header className="finance-header" style={{background:DARK,color:"white",borderBottom:`3px solid ${GOLD}`,padding:"22px 6vw",display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><div><h1 style={{fontFamily:"Playfair Display,Georgia,serif",fontSize:28,margin:0}}>Governance & Operations Center</h1><p style={{margin:"5px 0 0",opacity:.78,fontSize:12}}>Finance Ledger · locked records, receipts, contributions, expenses, adjustments and audit trail</p></div><div className="finance-actions" style={{display:"flex",gap:8}}><button style={{...secondary,background:"transparent",color:"white",borderColor:"rgba(255,255,255,.3)"}} onClick={()=>location.assign("/admin/operations")}><ArrowLeft size={14}/> Governance</button><button style={{...secondary,background:"transparent",color:"white",borderColor:"rgba(255,255,255,.3)"}} onClick={()=>location.assign("/admin")}><ArrowLeft size={14}/> Main Admin</button></div></header>
    <main className="finance-main" style={{padding:"24px 6vw 60px"}}>
      ...
      <div style={{padding:"10px 13px",border:`1px solid ${isSuperAdmin?"#d8c483":"#d6e1da"}`,background:isSuperAdmin?"#fff9e9":"#f3f8f5",borderRadius:9,marginBottom:14,fontSize:12,color:"#4f5f55",display:"flex",gap:8,alignItems:"center"}}><Lock size={15} color={GREEN}/><span><b>Ledger Lock:</b> every posted transaction is permanent. Finance staff and normal admins can post and view records, but cannot alter or remove a posted entry. Only Super Admin can correct, archive or restore, and every such action is written to the audit history.</span></div>
      ...
      <section style={{...panel,marginBottom:18}}><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"flex-start",flexWrap:"wrap"}}><div><h2 style={{color:GREEN,fontFamily:"Playfair Display,serif",margin:"0 0 5px"}}>{editingId?"Super Admin Correction":"Post Revenue, Expense or Adjustment"}</h2><p style={{fontSize:12,color:"#777",margin:"0 0 14px"}}>Annual membership defaults to Rs. 1,000, life membership to Rs. 3,000 and Executive Committee annual contribution to Rs. 12,000. New entries are locked immediately after posting.</p></div>{editingId&&<span style={{padding:"4px 9px",background:"#fff4cf",color:"#765b09",borderRadius:999,fontSize:11,fontWeight:800}}>Super Admin correction · audited</span>}</div>
