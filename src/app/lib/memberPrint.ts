...
export async function printMemberFormV2(m: Member) {
  ...
  const organization = [m.institutionName, m.businessName].filter(Boolean).join(" · ");
  const referrer = m.referrerMember ? [m.referrerMember.fullName, m.referrerMember.memberNo].filter(Boolean).join(" · ") : "Not provided";
  const communityCell = m.memberCell === "women" ? "Women's Cell" : "Men's Cell";
  const additionalCount = Array.isArray(m.additionalPhotos) ? m.additionalPhotos.length : 0;
  const documentStatus = [
    `Photo ${m.photoUrl ? "✓" : "Missing"}`,
    `CNIC Front ${m.cnicFrontUrl ? "✓" : "Missing"}`,
    `CNIC Back ${m.cnicBackUrl ? "✓" : "Missing"}`,
    `Payment Proof ${m.paymentProofUrl ? "✓" : "Missing"}`,
  ...
  const makePage = (copyLabel: string, officeRecord: boolean) => `<section class="sheet">
    ...
    ${officeRecord ? `<div class="office-verify"><div><span>Community Cell</span><b>${escapeHtml(communityCell)}</b></div><div><span>Organization / Business</span><b>${escapeHtml(organization || "Not provided")}</b></div><div><span>Referrer</span><b>${escapeHtml(referrer)}</b></div><div class="docs"><span>Document Checklist</span><b>${escapeHtml(documentStatus)}</b></div></div>` : ""}
    <div class="copy-note">${officeRecord ? `<b>Office Record Reference:</b> Internal Form Serial ${escapeHtml(m.formNo || "—")}. Retain this copy after document review, payment verification and signatures.` : `Member copy. Valid subject to the Constitution, rules and current membership status of Anjuman-e-Araian Faisalabad.`}</div>
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(m.memberNo || m.formNo)} - Membership Form</title><style>
    ...
    .meta{display:grid;grid-template-columns:1.35fr 1.15fr .8fr 1fr;gap:4mm;border-top:1px solid #d2bb67;border-bottom:1px solid #d2bb67;background:#fffdf7;padding:1.8mm 2.5mm;margin:1.5mm 0 3mm}.section-title,.payment-title,.signature-title{font-size:8.6px;text-transform:uppercase;letter-spacing:.12em;font-weight:800;color:#174e32;border-bottom:1px solid #d9dfdb;padding-bottom:1.2mm;margin-bottom:1mm}.payment-title{margin-top:3mm}.signature-title{margin-top:2.3mm}
    ...
    .declaration{margin-top:3.5mm;border:1px solid #4a544e;display:grid;grid-template-columns:34mm 1fr;min-height:37mm}.declabel{background:#174e32;color:white;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:3mm;font-family:"Noto Nastaliq Urdu","Noto Naskh Arabic",Tahoma,Arial,sans-serif;font-size:10px;font-weight:800;line-height:1.65}.declabel span{font-family:Arial,"Segoe UI",sans-serif;font-size:6.5px;letter-spacing:.07em}.decltext{font-family:"Noto Nastaliq Urdu","Noto Naskh Arabic",Tahoma,Arial,sans-serif;direction:rtl;unicode-bidi:plaintext;text-align:right;padding:3mm 4mm;font-size:9.1px;line-height:1.82;word-spacing:.06em;color:#26332c}
    .paygrid{background:#fbf8ef;border:1px solid #d8c68b;padding:1.1mm 2.5mm}.paygrid .field{min-height:8.2mm}.office-verify{display:grid;grid-template-columns:.7fr 1.25fr 1.15fr;gap:2mm 4mm;margin-top:2mm;padding:1.4mm 2.2mm;border:1px solid #dbe4de;background:#f8fbf9}.office-verify>div{min-width:0}.office-verify .docs{grid-column:1/-1;border-top:1px solid #e4e9e6;padding-top:1mm}.office-verify span{display:block;font-size:5.7px;text-transform:uppercase;color:#68736c;letter-spacing:.03em}.office-verify b{display:block;font-size:6.8px;color:#26372e;margin-top:.4mm;line-height:1.3;word-break:break-word}.copy-note{margin-top:1.8mm;padding:1.4mm 2.5mm;border-left:3px solid #c7a249;background:#fafbf9;font-size:6.6px;line-height:1.4;color:#536159}
    .signatures{display:grid;grid-template-columns:repeat(4,1fr);gap:5mm;text-align:center;align-items:end}.signature-box{min-width:0}.signature-space{height:8mm}.signature-line{border-bottom:1px solid #53655a;margin:0 1.5mm 1.2mm}.signature-box b{display:block;font-size:7.3px;color:#25372d;line-height:1.25;min-height:3.5mm}.signature-box small{display:block;font-size:5.8px;color:#6e766f;margin-top:.5mm;line-height:1.25;min-height:3.5mm}
