...
async function prepareUpload(file: File) {
  ...
  const prepared = isImage ? await optimizeImageFile(file, { maxWidth: 1920, maxHeight: 1920, quality: 0.86 }) : file;
  if (prepared.size > 4 * 1024 * 1024) {
    throw new Error(isImage ? "Image is still larger than 4 MB after optimization. Please use a smaller source image." : "File must be 4 MB or smaller.");
  }
  return prepared;
}
