      ...
      <div style={{ backgroundColor: GREEN, padding: "8px 0" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 8, fontSize: 13 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            {settings.contactPhone && <a href={`tel:${settings.contactPhone}`} style={{ display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,0.9)", textDecoration: "none" }}><Phone size={13} /> {settings.contactPhone}</a>}
            {settings.contactEmail && <a href={`mailto:${settings.contactEmail}`} style={{ display: "flex", alignItems: "center", gap: 6, color: "rgba(255,255,255,0.9)", textDecoration: "none" }}><Mail size={13} /> {settings.contactEmail}</a>}
          </div>
      ...
      <div style={{ backgroundColor: "white", borderBottom: "1px solid #fde6f1" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", justifyContent: "space-between", minHeight: 80 }}>
          <Link to="/" style={{ display: "flex", alignItems: "center", gap: 14, textDecoration: "none" }}>
            ...
            <div><div style={{ color: GREEN, fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, lineHeight: 1.2 }}>Anjuman e Araian</div><div style={{ color: "#888", fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase" }}>Faisalabad — Est. 1947</div></div>
          </Link>
          ...
          <nav style={{ display: "flex", alignItems: "center", gap: 2 }} className="desktop-nav">
            {navLinks.map((link) => <div key={link.label} style={{ position: "relative" }} onMouseEnter={() => link.children && setOpenDropdown(link.label)} onMouseLeave={() => setOpenDropdown(null)}>
              <Link to={link.to} style={{ display: "flex", alignItems: "center", gap: 3, padding: "8px 11px", borderRadius: 4, fontSize: 13, fontWeight: 600, textDecoration: "none", color: isActive(link.to) ? "white" : "#3a3a3a", backgroundColor: isActive(link.to) ? GREEN : "transparent", whiteSpace: "nowrap" }}>{link.label}{link.children && <ChevronDown size={12} />}</Link>
            ...
            {member ? <Link to="/member/portal" style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", backgroundColor: GOLD, color: "#1a1a1a", borderRadius: 6, fontSize: 12, fontWeight: 700, textDecoration: "none" }}><UserCircle size={15} /> {member.fullName.split(" ")[0]}</Link> : <Link to="/member/login" style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 12px", border: `2px solid ${GREEN}`, color: GREEN, borderRadius: 6, fontSize: 12, fontWeight: 700, textDecoration: "none" }}><UserCircle size={15} /> Member Login</Link>}
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} style={{ display: "none", background: GREEN, border: "none", color: "white", cursor: "pointer", padding: 8, borderRadius: 6 }} className="mobile-menu-btn" aria-label="Toggle menu">{mobileOpen ? <X size={22} /> : <Menu size={22} />}</button>
        </div>

        {mobileOpen && <div style={{ backgroundColor: "white", borderTop: "1px solid #eee" }}>{navLinks.map((link) => <div key={link.label} style={{ borderBottom: "1px solid #f5f5f5" }}>
          <div style={{ display: "flex", alignItems: "center" }}>{link.children ? <button onClick={() => setMobileExpanded(mobileExpanded === link.label ? null : link.label)} style={{ flex: 1, textAlign: "left", background: "none", border: "none", padding: "12px 24px", fontSize: 14, fontWeight: 600, color: isActive(link.to) ? GREEN : "#222", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", width: "100%", fontFamily: "inherit" }}><span>{link.label}</span><ChevronDown size={16} style={{ transform: mobileExpanded === link.label ? "rotate(180deg)" : "none", color: GREEN, transition: "transform .2s" }} /></button> : <Link to={link.to} style={{ flex: 1, display: "block", padding: "12px 24px", fontSize: 14, fontWeight: 600, textDecoration: "none", color: isActive(link.to) ? GREEN : "#222" }} onClick={() => setMobileOpen(false)}>{link.label}</Link>}</div>
          {link.children && mobileExpanded === link.label && <div style={{ borderLeft: `3px solid ${GOLD}`, backgroundColor: "#fafafa" }}>{link.children.map((c) => <Link key={c.to} to={c.to} style={{ display: "block", padding: "10px 32px", fontSize: 13, textDecoration: "none", color: "#555", borderBottom: "1px solid #f0f0f0" }} onClick={() => setMobileOpen(false)}>{c.label}</Link>)}</div>}
        </div>)}</div>}
      </div>
