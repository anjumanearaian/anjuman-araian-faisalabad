...
export function PasswordlessSignIn({
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async ({
          ...
          try {
            ...
            const session = await apiClient<Session>("/auth/google", {
              method: "POST",
    ...
    try {
      ...
      setNotice(
        data.devOtp
          ...
          : "A 6-digit code has been sent. It expires in 10 minutes."
      );
    } catch (e: any) {
  ...
  const verify = async () => {
    ...
    try {
      const session = await apiClient<Session>("/auth/email/verify-otp", {
        method: "POST",
        body: JSON.stringify({
    ...
    <div>
      ...
      <div style={{ position: "relative" }}>
        ...
        <input
          ...
          placeholder="Gmail, Hotmail, Yahoo or any email"
          autoComplete="email"
        />
      </div>
      ...
      >
        No password required. Your verified email securely restores saved
        forms on any device.
      </p>
    </div>
  );
