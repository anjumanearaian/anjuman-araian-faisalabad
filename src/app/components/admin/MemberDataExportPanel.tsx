...
function memberSearchValues(member: Member) {
  return [
    member.fullName,
    member.fatherName,
    member.memberNo,
...
export function MemberDataExportPanel({ members }: { members: Member[] }) {
  ...
  const visible = useMemo(
    () => smartSearchSort(members, query, memberSearchValues, (member) => member.fullName || member.memberNo || ""),
    [members, query],
  );
  ...
  const exportAllExcel = () => {
    setError("");
    try {
      const alphabetical = smartSearchSort(members, "", memberSearchValues, (member) => member.fullName || member.memberNo || "");
      exportMemberRecords(alphabetical, "xlsx");
    } catch (e: any) {
  ...
  return <>
    <button onClick={exportAllExcel} disabled={!members.length} title="Download the complete member registry as one Excel file, sorted by member name" style={{ ...primaryButton, opacity: members.length ? 1 : .55 }}><FileSpreadsheet size={14} /> Export All Excel</button>
    ...
    {open && <div style={{ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(0,0,0,.58)", display: "grid", placeItems: "center", padding: 16 }}>
      <div style={{ width: "min(760px,96vw)", maxHeight: "90vh", background: "white", borderRadius: 14, boxShadow: "0 24px 80px rgba(0,0,0,.28)", overflow: "hidden" }}>
        <div style={{ padding: "18px 20px", borderBottom: "1px solid #e7ece8", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}><div><h3 style={{ margin: 0, color: GREEN, fontFamily: "'Playfair Display', serif" }}>Download Member Database</h3><p style={{ margin: "4px 0 0", color: "#777", fontSize: 12 }}>Smart search is word-order independent. Type 3-4 letters such as “mun”, “khal”, “shouq” or multiple words such as “shouq khalid”. Results are ranked and then kept name-wise.</p></div><button onClick={() => setOpen(false)} style={{ border: 0, background: "transparent", cursor: "pointer", color: "#777" }}><X size={20} /></button></div>
        <div style={{ padding: 20 }}>
          {error && <div style={{ padding: 10, marginBottom: 12, background: "#fee2e2", color: "#b91c1c", borderRadius: 7, fontSize: 12 }}>{error}</div>}
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search name or partial name, registration no., CNIC, phone, city..." autoComplete="off" style={{ width: "100%", boxSizing: "border-box", padding: "10px 12px", border: "1px solid #d9e2dc", borderRadius: 8, fontSize: 12 }} />
          <div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}><div style={{ fontSize: 11, color: "#666" }}>{selected.size ? `${selected.size} selected` : `${visible.length} matching record${visible.length === 1 ? "" : "s"}`}</div><div style={{ display: "flex", gap: 7 }}><button onClick={toggleVisible} style={smallButton}>{allVisibleSelected ? "Clear Search Results" : "Select Search Results"}</button>{selected.size > 0 && <button onClick={() => setSelected(new Set())} style={smallButton}>Clear Selection</button>}</div></div>
          <div style={{ marginTop: 12, border: "1px solid #e5e7eb", borderRadius: 9, maxHeight: 360, overflow: "auto" }}>{visible.map((m) => <label key={m.id} style={{ display: "grid", gridTemplateColumns: "22px 1fr auto", gap: 10, alignItems: "center", padding: "10px 12px", borderBottom: "1px solid #f0f1f0", cursor: "pointer" }}><input type="checkbox" checked={selected.has(m.id)} onChange={() => toggle(m.id)} /><div><strong style={{ color: "#24392d", fontSize: 12 }}>{m.fullName}</strong><div style={{ color: "#888", fontSize: 10, marginTop: 2 }}>{[m.fatherName, m.cnic, m.phone].filter(Boolean).join(" · ")}</div></div><span style={{ color: GREEN, fontSize: 10, fontWeight: 800 }}>{m.memberNo}</span></label>)}{visible.length === 0 && <div style={{ padding: 30, textAlign: "center", color: "#999", fontSize: 12 }}>No matching member records.</div>}</div>
