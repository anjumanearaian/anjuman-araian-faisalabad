...
export default function ActivitiesSection() {
  ...
  const countClass = items.length === 1 ? "home-news-grid--count-1" : items.length === 2 ? "home-news-grid--count-2" : "home-news-grid--count-3";
  ...
  return <section className="home-section home-updates-section">
    <div className="home-shell">
      ...
      {items.length ? <div className={`home-news-grid home-news-grid--landscape ${countClass}`}>{items.map((item) => {
        ...
        return <article className="home-news-card home-news-card--uniform" key={item.id} data-content-id={item.id}>
          <div className="home-news-media">
            {item.images?.[0] ? <ResponsiveImage src={item.images[0]} alt={`${item.title} featured image`} widthHint={640} sizes="(max-width:820px) 100vw, 50vw" /> : <div className="home-news-fallback"><Newspaper size={28} /><span>Anjuman-e-Araian Faisalabad</span></div>}
          </div>
